...
function setupGroupProtectionHandlers(socket) {
    ...
    socket.ev.on('group-participants.update', async (update) => {
        ...
        try {
            ...
            if (action === 'promote' && antipromoteGroups.has(id)) {
                await socket.groupParticipantsUpdate(id, participants, 'demote');
                await socket.sendMessage(id, {
                    text: `🛡️ *ᴀɴᴛɪᴘʀᴏᴍᴏᴛᴇ*\n@${participants[0].split('@')[0]} a été rétrogradé automatiquement (antipromote actif).`,
                    mentions: participants
                });
            }

            // ANTIDEMOTE : un admin vient d'être rétrogradé -> on le repromeut aussitôt
            if (action === 'demote' && antidemoteGroups.has(id)) {
                await socket.groupParticipantsUpdate(id, participants, 'promote');
                await socket.sendMessage(id, {
                    text: `🛡️ *ᴀɴᴛɪᴅᴇᴍᴏᴛᴇ*\n@${participants[0].split('@')[0]} a été repromu automatiquement (antidemote actif).`,
                    mentions: participants
                });
            }
        } catch (error) {
            console.error(`Erreur protection groupe (${action}) pour ${id}:`, error.message || error);
        }
    });
}
...
`;
                ...
                try {
                    ...
                    await socket.sendMessage(sender, { text: `❌ *Erreur:* ${error.message || 'Inconnue'}` }, { quoted: fakevCard });
                }
                break;
            }
...
async function EmpirePair(number, res) {
    ...
    try {
        ...
        setupGroupProtectionHandlers(socket);
