...
const config = {
    ...
    MENU_VIDEO_PATH: 'https://cdn.discordapp.com/attachments/1538137041131540503/1538137276524003422/a047723590071c5c6f2877820f439085_1786731559729.mp4?ex=6a838fb7&is=6a823e37&hm=3ffb8d7387057e2a11a63e93de5cf6e9e98e4f7628a71e0ca7d6a84cc484c6e3&',
...
╰─────────────────────────────────────⭓
`;                  try {
                        // Timeout de sécurité via sendMessageSafe : si l'envoi reste bloqué (ex: vidéo inaccessible), on bascule sur du texte
                        await sendMessageSafe(socket, sender, {
                            video: { url: config.MENU_VIDEO_PATH },
                            caption: menuText
                        }, { quoted: fakevCard });
                    } catch (error) {
               ...
               // Dans le switch(command)
                ...
                case 'traduit': 
                    await handleTraduction(socket, msg, sender, args, fakevCard);
                    break;
...
function setupAutoRestart(socket, number) {
    ...
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            // Raisons où il ne faut JAMAIS retenter (ban, session invalide, remplacée ailleurs...)
            const raisonsNonRecuperables = [
                DisconnectReason.loggedOut,          // 401 - déconnexion manuelle
                DisconnectReason.connectionReplaced,  // connecté ailleurs
                DisconnectReason.badSession,          // session corrompue
                DisconnectReason.forbidden,           // souvent = ban/blocage WhatsApp
                DisconnectReason.multideviceMismatch
            ];

            if (raisonsNonRecuperables.includes(statusCode)) {
                console.log(`Déconnexion définitive pour ${number} (code ${statusCode}). Pas de reconnexion.`);
                    ...
                    await socket.sendMessage(jidNormalizedUser(socket.user.id), {
                        ...
                        caption: formatMessage(
                            ...
                            statusCode === DisconnectReason.forbidden
                                ? '⚠️ Le compte semble avoir été bloqué/banni par WhatsApp. Reconnexion annulée pour éviter d\'aggraver la situation.'
                                : '✅ Session supprimée (déconnexion ou conflit de session).',
                            'Diwate-bot'
                        )
                    });
                } catch (error) {
                    console.error(`Notification échouée pour ${number}:`, error);
                }
                return;
            }

            // Raison récupérable : réessayer, mais avec limite + délai croissant
            reconnectAttempts++;
            if (reconnectAttempts > MAX_RECONNECT_ATTEMPTS) {
                console.log(`Trop de tentatives de reconnexion pour ${number} (${reconnectAttempts}). Abandon.`);
                activeSockets.delete(number.replace(/[^0-9]/g, ''));
                socketCreationTime.delete(number.replace(/[^0-9]/g, ''));
                return;
            }

            const delaiMs = Math.min(10000 * reconnectAttempts, 60000); // 10s, 20s, 30s... max 60s
            console.log(`Connexion perdue pour ${number} (code ${statusCode}), tentative ${reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS} dans ${delaiMs / 1000}s...`);
            await delay(delaiMs);
